...
def backupBeforeSave(filepath):
    ...
    if os.path.isfile(filepath):  # if a file exists with the same name, rename existing file
        ...
        os.rename(filepath,
                  os.path.join(address_filename[0], nowString() + "_" + address_filename[1]))
...
def addLeadingZeros(path, n_digits=6):
...
def generate_CTMC_Matrix(T_stick, T_unstick, dt):
    P = np.zeros([2, 2])
    r = 1. / T_stick + 1. / T_unstick  # combined rate
    phi = np.exp(-r * dt)
    P[0, 0] = T_stick + T_unstick * phi
    P[0, 1] = T_unstick * (1 - phi)
    P[1, 0] = T_stick * (1 - phi)
    P[1, 1] = T_unstick + T_stick * phi
    P /= (T_stick + T_unstick)
    return P
